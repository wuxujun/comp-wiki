...
- Material under `raw/` remains verbatim in its original language and is never translated in place.
- Curated pages under `wiki/`, including source notes for English material, use Chinese as this repository's default synthesis language unless a page is explicitly intended for an English-speaking audience.
- Source notes for non-Chinese material must record the source language and preserve original titles, technical terms, proper nouns, code identifiers, and any necessary short quotations in the source language.
- Do not import or translate-copy a copyrighted full text merely to make it searchable; summarize it in `wiki/` and retain a stable identifier or provenance link instead.
- `wiki/index.md`, `wiki/log.md`, and `mocs/` may use English headers for consistency; their descriptions follow the repository's default Chinese synthesis language.
